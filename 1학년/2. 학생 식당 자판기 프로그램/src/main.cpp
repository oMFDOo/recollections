...
int returnPrice(int price) {
	...
	if (numberOfDish < 1) {		// 음수 처리
		numberOfDish *= -1;
	}
	return numberOfDish * price;	// 메뉴 총 가격 반환
}
...
void printResultPrice(int price) {
	printf("결제하실 총 금액은 %d원 입니다.", price);
	printDividingLine();
}
...
int main() {
	...
	printResultPrice(price);				// 메뉴 출력		:		총가격 출력
	change = inputMoney(price);				// 금액 투입		:		거스름돈 리턴
	calculateChange(change);				// 거스름돈 계산	:		거스름돈 개수 계산 및 출력
